...
PAPERS_DIR="${VAULT_ROOT}/Literature"
IMAGES_BASE="${VAULT_ROOT}/Literature/images"
```
...
- 论文标题："论文标题"
- 文件路径：直接路径到现有笔记
- **citekey格式**：`[第一作者姓][年份][标题关键词]`，如 `wang2024metmap`、`deng2024pentestgpt`

### 1.2 检查现有笔记

1. **搜索已有笔记**
   - 按arXiv ID在`Literature/`目录中搜索
   - 按citekey或标题匹配
   - 如果找到，读取该笔记
...
2. **获取PDF内容和图片**
   - 使用WebFetch获取PDF
   - **重要**：提取论文中的所有图片
   - 保存图片到`Literature/images/[citekey]/`
...
```bash
# 复制figures目录到目标位置
cp /tmp/paper_analysis/*.{pdf,png,jpg,jpeg} "$IMAGES_BASE/[citekey]/" 2>/dev/null

# 列出复制的内容
ls "$IMAGES_BASE/[citekey]/"
```
...
```bash
# 生成 citekey：[第一作者姓小写][年份][标题关键词小写]
# 例如：wang2024metmap、deng2024pentestgpt、liu2026AgentSkillsWild
CITEKEY="[第一作者姓小写][年份][标题关键词]"

PAPERS_DIR="${VAULT_ROOT}/Literature"
NOTE_PATH="${PAPERS_DIR}/${CITEKEY}.md"
IMAGES_DIR="${PAPERS_DIR}/images/${CITEKEY}"
...
**注意**：优先使用论文中原有的架构图。论文中的图通常更准确、更权威。
...
![实验结果图|800](images/[citekey]/实验结果.png)
...
```bash
# 搜索已有笔记
find "${VAULT_ROOT}/Literature" -name "*${PAPER_ID}*" -o -name "*${CITEKEY}*" -type f
```
...
1. **读取图谱数据**
   - 文件路径：`$OBSIDIAN_VAULT_PATH/Literature/PaperGraph/graph_data.json`
...
1. **frontmatter格式（重要 - CI PDF 兼容）**：
   - 前三行必须是 `toc: true`、`documentclass: "ctexart"`、`classoption: "UTF8"`（CI PDF 转换必需）
   - 所有字符串值必须用双引号包围
   ```yaml
   ---
   toc: true
...
2. **图片路径**：使用相对路径 `images/[citekey]/filename.png`（相对于 Literature/ 目录）
   - **图片语法**：必须使用标准 markdown `![描述|宽度](images/[citekey]/filename.png)`
   - **禁止使用** `![[]]` wikilink 语法（CI PDF 转换不支持）
   - **路径安全**：目录名不得包含 `?`、`%`、`#` 等特殊字符
3. **wikilinks**：使用`[[论文名]]`格式（仅用于论文间引用，不用于图片）
4. **笔记文件名**：使用 citekey 格式 `Literature/[citekey].md`，如 `Literature/wang2024metmap.md`
...
1. **Frontmatter 前三行**：必须是 `toc: true`、`documentclass: "ctexart"`、`classoption: "UTF8"`
...
**图文并茂**：论文中的所有图片都要用上
- **保存到正确位置**：`Literature/images/[citekey]/`
- **图片索引**：生成`images/[citekey]/index.md`索引所有图片
