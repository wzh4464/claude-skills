...
**重要**：
- **图片保存目录**：`$OBSIDIAN_VAULT_PATH/Literature/images/[citekey]/`（citekey 如 `wang2024metmap`、`deng2024pentestgpt`）
- **笔记中引用路径**：`images/[citekey]/filename.png`（相对于 Literature/ 目录）
- **图片语法**：必须使用标准 markdown 语法 `![描述|宽度](images/citekey/filename.png)`，**禁止**使用 `![[]]` wikilink 语法（CI PDF 转换不支持）
- **路径安全**：目录名不得包含 `?`、`%`、`#` 等特殊字符，使用 citekey 作为目录名
...
- 论文标题
- 图片目录：`Literature/images/[citekey]/`
- 图片索引：`Literature/images/[citekey]/index.md`
- 核心图片：`images/final_results_combined_page1.png`等（前3-5张）
- 图片来源标识（arxiv-source、pdf-figure、pdf-extraction）

# 重要规则

- **保存到正确目录**：`Literature/images/[citekey]/`
- **生成索引文件**：记录所有图片信息和来源
- **图片质量**：确保清晰度足够高
