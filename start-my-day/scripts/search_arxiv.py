...
ARXIV_CATEGORY_KEYWORDS = {
    ...
    "cs.SE": "software engineering agent coding",
...
def search_semantic_scholar_hot_papers(
    ...
    # 添加 Semantic Scholar API Key（如果设置了环境变量）
...
def calculate_relevance_score(
    ...
    for domain_name, domain_config in domains.items():
        ...
        # 如果没有任何关键词匹配，跳过该领域（类别匹配不再单独算分）
        if not has_keyword_match:
            continue

        score = keyword_score

        # 类别匹配作为小幅加分（仅在关键词已匹配的前提下）
        domain_categories = domain_config.get('arxiv_categories', [])
        for cat in domain_categories:
            if cat in categories:
                score += RELEVANCE_CATEGORY_MATCH_BOOST * 0.3  # 降低类别权重
